...
def initialize_database(character_file, model_name='ollama'):
    ...
    if cursor.fetchone() is None:
        ...
        ''')
        conn.commit()
        logging.info(f"Database initialized for character: {character_file} with model: {model_name}")
    else:
        ...
        if missing_columns:
            ...
            logging.info(f"Database already exists and has all required columns for character: {character_file}")
    
    conn.close()

# Function to add memory entry
def add_memory_entry(character_file, new_entry, model_name='ollama'):
    if new_entry.get('query_embedding') is None or new_entry.get('response_embedding') is None:
...
def get_query(query, character_file, character_data, model_name='ollama'):
    ...
    try:
        result = subprocess.run(MODEL_COMMAND, input=messages, capture_output=True, text=True, encoding='utf-8')
