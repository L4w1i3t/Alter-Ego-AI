...
def truncate_text(text, max_length=MAX_TOKEN_LENGTH):
    ...
    tokenized_text = tokenizer(text, add_special_tokens=True, truncation=True, max_length=max_length, return_tensors="pt")
...
def detect_emotions(texts, score_threshold=0.01):
    ...
    for i, text in enumerate(truncated_texts):
        print(f"Truncated Text {i}: {text[:50]}...")  # Print first 50 characters for brevity
